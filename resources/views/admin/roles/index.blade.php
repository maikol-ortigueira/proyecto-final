<x-app-layout>
  ...
  <div class="container m-auto mt-5">
    {{-- Botón para añadir nuevos roles --}}
    <div class="my-6 text-right">
      <a href="{{ route('admin.roles.create') }}"
        class="rounded bg-blue-500 py-1 px-2 capitalize text-blue-100 hover:bg-blue-800">{{ __('new') . ' ' . __('role') }}</a>
    </div>
              ...
              <tbody>
                @foreach ($roles as $item)
                  <tr
                    ...
                    {{-- Roles protegidos y que no se pueden editar ni borrar --}}
                    @php
                      $protected = ['superadmin', 'editor', 'registrado'];
                    @endphp

                    <td class="whitespace-nowrap py-4 px-6 text-right text-sm font-medium">
                      @if (!in_array($item->nombre, $protected))
                        <a href="{{ route('admin.roles.edit', ['rol' => $item]) }}"
