<x-app-layout>
  <x-slot name="header">
    <h2 class="text-xl font-semibold capitalize leading-tight text-gray-800">{{ __('new') . ' ' . __('rol') }}
    </h2>
  </x-slot>
  ...
  <div class="container m-auto py-6">
    {{-- Formulario --}}
    <form action="{{ route('admin.roles.store') }}" id="rolForm" method="post">
      @csrf
      @method('POST')
      ...
      <div class="my-10 text-left">
        <a href="{{ route('admin.roles.index') }}" class="rounded bg-red-500 py-1 px-2 capitalize text-red-100 hover:bg-red-800" >{{ __('close') }}</a>
